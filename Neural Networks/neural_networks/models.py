...
class NeuralNetwork(ABC):
    def __init__(
        ...
        self.n_layers = len(layer_args) # numero de capas
        self.layer_args = layer_args # argumentos para las capa (vease train_ffnn.py)
        self.logger = logger # al principio None
        self.epoch_log = {"loss": {}, "error": {}} # creo que esto es para guardar medidas

        self.loss = initialize_loss(loss) # inicializa la función para medir la perdida usando entropía (vease losses.py linea 23)
        self.optimizer = initialize_optimizer(**optimizer_args) # al llamar ésta función se regresará la clase que implementa SGD.
        self._initialize_layers(layer_args) # llama la función dentro de ésta misma clase é inicializa las capas y retorna un objeto tipo lista que depende del tipo de capas especificadas (FullyConnected, Con2D, o Pool2D)
    ...
    def _initialize_layers(self, layer_args: Sequence[AttrDict]) -> None:
        ...
        Parametros
        layer_args : Objeto tipo Sequence que contiene los parametros para las capas al ser inicializadas
