...
class SoftMax(Activation):
    ...
    def forward(self, Z: np.ndarray) -> np.ndarray:
        ...
        """
        ### YOUR CODE HERE ###
       # treat Z as s in the softmax activation function (Z = Wh)
       # get the maximum value at each row
        _m = np.max(Z, axis=1, keepdims=True)
        # subtract every row in Z by the maximum value at each row
        _Z = Z - _m
        # find softmax numerator
        num = np.exp(_Z)
        # find softmax denominator
        denom = np.sum(np.exp(_Z), axis=1, keepdims=True)
    ...
    def backward(self, Z: np.ndarray, dY: np.ndarray) -> np.ndarray:
        ...
        """
        Y = self.forward(Z)
        dLdZ = Y * (dY - np.sum(Y*dY, axis=1, keepdims=True))
        return dLdZ
